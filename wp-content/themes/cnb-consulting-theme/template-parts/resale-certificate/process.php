...
$steps = array(
    array(
        'title' => 'Eligibility & nexus review',
        'description' => 'Confirm entity details, sales footprint, and vendor requirements. We collect supporting documents and identify nexus responsibilities.',
        'cta' => 'Confirm intake checklist'
    ),
    array(
        'title' => 'Application & exemption prep',
        'description' => 'State forms, multi-jurisdiction certificates, and supplier-specific exemption letters drafted with your business data.',
        'cta' => 'Approve exemption packets'
    ),
    array(
        'title' => 'Submission & vendor onboarding',
        'description' => 'We submit to state tax agencies, coordinate vendor approvals, and upload everything to your compliance dashboard.',
        'cta' => 'Activate vendor approvals'
    ),
    array(
        'title' => 'Renewal & compliance monitoring',
        'description' => 'Automated reminders, document vault updates, and continuous support keep certificates active and audit-ready.',
        'cta' => 'Review renewal schedule'
    ),
);
?>

<section id="resale-process" class="relative bg-slate-950 py-20 lg:py-24 text-white" x-data="{ activeStep: 0 }">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col gap-6 text-center">
            ...
            <h2 class="text-3xl md:text-4xl lg:text-5xl font-semibold"><?php echo esc_html__('Resale certificate approval in four managed checkpoints', 'cnb-consulting-theme'); ?></h2>
            <p class="mx-auto max-w-3xl text-base text-white/70"><?php echo esc_html__('Stay focused on sourcing while our team handles every filing, vendor interaction, and compliance deadline. Hover or tap to see the details behind each milestone.', 'cnb-consulting-theme'); ?></p>
        </div>
                ...
                <?php foreach ($steps as $index => $step) : ?>
                    <button type="button"
                        ...
                        <span class="flex items-center gap-3">
                            <span class="flex h-8 w-8 items-center justify-center rounded-full border border-current text-xs font-semibold"><?php echo esc_html($index + 1); ?></span>
                ...
                <?php foreach ($steps as $index => $step) : ?>
                    <div x-show="activeStep === <?php echo esc_js($index); ?>" x-transition.opacity.duration.400ms>
                        <div class="flex items-center gap-4 text-white/70">
                            <span class="flex h-10 w-10 items-center justify-center rounded-full bg-cnb-secondary/20 text-sm font-semibold text-cnb-secondary"><?php echo esc_html($index + 1); ?></span>
