...
<li class="relative" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
    <a href="<?php echo esc_url(home_url('/services/')); ?>" class="cnb-nav-link flex items-center space-x-2 text-slate-200 hover:text-cnb-secondary font-semibold transition-all duration-300 text-base tracking-wide hover:-translate-y-0.5">
        <span><?php echo __('Services', 'cnb-consulting-theme'); ?></span>
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute left-0 top-full mt-3 w-80 bg-white rounded-xl shadow-2xl z-50 border border-slate-200 max-h-[520px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent scrollbar-thumb-rounded-full hover:scrollbar-thumb-slate-700 scrollbar-corner-transparent" style="display: none;">
